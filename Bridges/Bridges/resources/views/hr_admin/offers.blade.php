  ...
  <div class="cp-card">
    <div class="cp-card-header">
      <div class="d-flex align-items-center gap-2">
        ...
        <span class="badge-stage bs-pending">{{ $offers->count() }}</span>
      </div>
    </div>
        ...
        @forelse($offers as $offer)
          ...
          <div class="list-row">
            <div class="d-flex align-items-center gap-3 flex-wrap">
              ...
              <div class="av-init" style="width:40px;height:40px;font-size:.8rem;flex-shrink:0;">{{ $initials }}</div>
              <div>
                <div style="font-weight:500;">{{ $candidate->first_name }} {{ $candidate->last_name }}</div>
                <!-- Job metadata -->
                <div class="d-flex align-items-center gap-3 mt-1 flex-wrap" style="font-size:.78rem;color:var(--muted);">
                  <span><i class="bi bi-briefcase me-1"></i>{{ $offer->job->title }}</span>
                  <span><i class="bi bi-building me-1"></i>{{ $offer->job->department }}</span>
                  <span><i class="bi bi-currency-dollar"></i>{{ $offer->job->salary_range ?? 'Negotiable' }}</span>
                </div>
              </div>
            </div>
            ...
            <div class="d-flex align-items-center gap-2 flex-wrap">
              ...
              <button class="btn-outline-cp btn-sm-cp"
                      onclick="openEditOfferModal('{{ addslashes($candidate->first_name . ' ' . $candidate->last_name) }}', '{{ addslashes($offer->job->title) }}', '{{ addslashes($offer->job->salary_range ?? 'Negotiable') }}', '2024-03-01', 'Health insurance, 401k matching, 20 days PTO', '')">
                <i class="bi bi-pencil"></i> Edit
              </button>
              <button class="btn-outline-cp btn-sm-cp"
                      onclick="openViewOfferModal('{{ addslashes($candidate->first_name . ' ' . $candidate->last_name) }}', '{{ addslashes($offer->job->title) }}', '{{ addslashes($offer->job->department) }}', '{{ addslashes($offer->job->salary_range ?? 'Negotiable') }}', 'March 1, 2024', 'Health insurance, 401k matching, 20 days PTO', '')">
