...
class HRAdminDashboardController extends Controller
{
     ...
     *
     * Fetches all offers from the database with their linked candidates.
     * Uses OfferService::getAllOffers() which eager-loads relations.
     * Uses OfferService::calculateOffer() for financial breakdown per offer.
     */
    public function offers()
    {
        ...
        // Build financial calculations map keyed by offer ID
        $calculations = $offers->mapWithKeys(
            fn (Offer $offer) => [$offer->id => $service->calculateOffer($offer)]
        );

        return view('hr_admin.offers', compact('user', 'offers', 'calculations'));
    }

    /**
     * OFFER EDIT FORM
     * Opens the edit form for a specific offer.
     */
    public function offerEdit(int $id)
    {
        $user    = Auth::user();
        $service = app(OfferService::class);
        $offer   = $service->findWithCandidate($id);
        $isValid = $service->manageOfferValidity($offer);

        return view('hr_admin.offer_edit', compact('user', 'offer', 'isValid'));
    }

    /**
     * OFFER UPDATE
     * Persists salary/bonus/expire_date changes.
     */
    public function offerUpdate(UpdateOfferRequest $request, int $id): RedirectResponse
    {
        $offer = Offer::findOrFail($id);
        app(OfferService::class)->updateOffer($offer, $request->validated());

        return redirect()->route('hr.offers')->with('success', 'Offer updated successfully.');
    }

    /**
     * SEND OFFER
     * Transitions offer status: draft → sent.
     */
    public function offerSend(int $id): RedirectResponse
    {
        $offer = Offer::findOrFail($id);

        try {
            app(OfferService::class)->sendOffer($offer);
            $flash = ['success' => 'Offer sent to candidate successfully.'];
        } catch (\RuntimeException $e) {
            $flash = ['error' => $e->getMessage()];
        }

        return redirect()->route('hr.offers')->with($flash);
    }
